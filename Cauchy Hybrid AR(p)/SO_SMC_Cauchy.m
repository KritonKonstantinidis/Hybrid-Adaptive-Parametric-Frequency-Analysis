function [A,log_likelihood,c] = SO_SMC_Cauchy(y,p,N,R,a,b)
...
R_e=1e-4; % variance of the variance of the cauchy 
...
for i=1:p
    ...
    for i=p+1:T         
        ...
        H(1:p)= y(i-1:-1:i-p)'; 
        H(p+1:2*p)=zeros(1,p);
        particles(1:N,1:p) = SO_cauchyrnd(particles_prev(1:N,1:p),c(:,i-1),N,p);
        particles(1:N,p+1:2*p)=normrnd(particles_prev(1:N,p+1:2*p),R_e,N,p);
