function specgram = SpectrogramEMD(a_slow,a_alpha,minfreq,maxfreq,varargin)

%% Input Arguments
% A: matrix of AR coefficients of each IMF (pxnXIMF)
...
%% Output Arguments
% specgram: structure containing information to produce spectrogram
%% Frequency decomposition
  P = size(a_slow,1); % number of coefficients
  N = size(a_slow,2); % number of observations 
  %%% Default values 
  opt.fs = 250;
  opt.index = 1:N;
  opt.psdres = 200;
  ...
  specgram.specgram_slow = zeros(length(omega),N); % num_freqs x observations
  specgram.specgram_alpha = zeros(length(omega),N); % num_freqs x observations
...
for n=1:N   % for each sample
    ...
    cp_slow= [1, -a_slow(:,index)']; % Slow frequency band
    cp_slow = cp_slow(:)';
    
    cp_alpha= [1, -a_alpha(:,index)']; % Alpha frequency band
    cp_alpha = cp_alpha(:)';
    ...
    for wi = 1:length(omega) % for each normalized frequency 
        ...
        powers = z.^(0:P);
        powers = powers(:)';
        specgram.specgram_slow(wi,index) = sqrt(0.00001)/(sum(powers.*cp_slow)); 
        specgram.specgram_alpha(wi,index) = sqrt(0.001)/(sum(powers.*cp_alpha));
    end
end

  specgram.specgram=20*log10(abs(specgram.specgram_slow+specgram.specgram_alpha).^2);
